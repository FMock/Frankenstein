...
AnimationDef::AnimationDef(int b, int c, int d, std::vector<Animation>& anims, const std::map<std::string, int>& aMap) : 
						   ...
						   animFPS(10.0f), animations(anims), numberOfAnimations(static_cast<int>(animations.size())), animationMap(aMap) {}
...
void AnimationDef::update(float deltaTime){
   ...
   if (frameTime > (1.0f/animFPS)){
	  ...
	  frameTime = static_cast<float>(fmod(frameTime, 1.0f / animFPS));
   }
}
...
void AnimationDef::setCurrentAnimation(int a){
   if(a >= 0 && a < static_cast<int>(animations.size()))
	   currentAnimation = a;
}
