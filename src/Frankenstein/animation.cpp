...
Animation::Animation(GLuint image, int a, int b, int c, const std::string& d, const int facingDir): m_image(image),
                   m_numberOfFrames(a), m_numberOfRows(b), m_frameDivision(1.0f/static_cast<float>(m_numberOfFrames)),
				   m_rowDivision(1.0f/static_cast<float>(m_numberOfRows)), m_startingRow(c), m_currentFrame(0),
				   m_currentRow(m_startingRow), m_s1(0.0f), m_s2(m_frameDivision), m_t1(m_currentRow*m_rowDivision), 
...
Animation::Animation(AnimationParameters& params, SpriteSheetParameters& spriteSheetParams, const std::string& name, const int facingDirection) :
   ...
   m_frameDivision(1.0f / static_cast<float>(m_spriteSheetInfo.m_spriteSheetWidth / m_spriteSheetInfo.m_frameWidth)),
   m_rowDivision(1.0f / static_cast<float>(m_spriteSheetInfo.m_spriteSheetHeight / m_spriteSheetInfo.m_frameHeight)),
...
void Animation::nextFrame(float frameTime, float animFPS){
   ...
   m_currentFrame += static_cast<int>(frameTime * animFPS);
